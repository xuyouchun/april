...
namespace X_ROOT_NS { namespace modules { namespace rt {
    ...
    {
        storage_type_t storage_type;
        msize_t size = __ctx.variable_size_of(def.type, &storage_type);

        if(__items.size() > 0 && def.index <= __items[__items.size() - 1].index)
        {
            for(int k = 0, count = X_ARRAY_SIZE(__groups); k < count; k++)
            {
            ...
            f_t f)
    {
        ...
        //msize_t new_offset = __set_offsets(offset, begin, end);
        ...
        if(child_index != __empty_msize)
        {
            ...
            **it_begin = __partial_t(child_index, end - g.items.begin());
            __arrange_variables(g, new_offset, it_begin, it_end);
            **it_begin = bk;

            for(__partial_iterator_t it = it_begin + 1; it < it_end; it++)
            {
                __partial_t * partial0 = *it;
                if(partial0->index <= partial->index)
                    break;

                if(partial0->index == child_index)
                    __arrange_variables(g, new_offset, it, it_end);
            }
        }
                                          ...
                                          __partial_iterator_t it_end, msize_t * out_end)
    {
        ...
        for(__partial_iterator_t it = it_begin + 1; it < it_end; it++)
        {
            ...
            if(partial0->index <= partial->index)
            {
                *out_end = partial0->begin;
                return min_index;
            }

            min_index = al::min(partial0->index, min_index);
        }
    ...
    {
        rt_type_t * type = __ctx.get_type(type_ref);
    ...
    void params_layout_t::append(rt_type_t * type, param_type_t param_type)
    {
        ...
        storage_type_t storage_type;
        msize_t unit_size = unit_size_of(type->get_variable_size(__ctx, &storage_type));
    ...
    void type_layout_t::append(rt_field_t * field)
    {
        ...
        rt_type_t * field_type = __ctx.get_type((*field)->type);
    ...
    void type_layout_t::commit()
    {
        ...
        __storage_type = storage_type_t::__default__;
        ...
        for(__item_t & it : __items)
        {
            ...
            if(__storage_type == storage_type_t::__default__)
